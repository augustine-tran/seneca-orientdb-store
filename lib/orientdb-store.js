...
module.exports = function (opts) {
    ...
    function execQuery(query, callback) {
        ...
        if (query.values) {
            ...
            db.query(query.text).then(function (rows) {
                return callback(null, {rows: rows});
            }).done(function() {
                server.close()
            })
...
//      })
    ...
    var savestm = function (ent) {
        ...
        fields.forEach(function (field) {
            var toSnakeCase = camelToSnakeCase(field);
            escapedFields.push('"' + escapeStr(toSnakeCase) + '"')
            values[toSnakeCase] = entp[field];
            params.push(':' + toSnakeCase)
        })
    ...
    var updatestm = function (ent) {
        ...
        fields.forEach(function (field) {
            if (['@class', '@type', '@rid'].indexOf(field) >= 0) {
            } else if (!_.isUndefined(entp[field])) {
                var toSnakeCase = camelToSnakeCase(field);
                values[toSnakeCase] = (entp[field]);
                params.push(escapeStr(toSnakeCase) + '=:' + toSnakeCase)
            }
        })
    ...
    var deletestm = function (qent, q) {
        ...
        stm.text = "DELETE VERTEX " + escapeStr(table) + wherestr
        stm.values = values
